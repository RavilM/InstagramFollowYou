                    ...
                    .then(response => {
                        ...
                        fetch(`${instUrl}graphql/query/?query_hash=c56ee0ae1f89cdbd1c89e2bc6b8f3d18&variables=${variables}`, {
                            ...
                            .then(text => {
                                ...
                                userId = parsedResponse.data.user.edge_follow.edges;
                                end_cursor = parsedResponse.data.user.edge_follow.page_info.end_cursor;

                                console.log('test', end_cursor);

                                setInterval(function () {
                                    const currentUserId = userId[0].node.id;

                                    fetch(`${instUrl}web/friendships/${currentUserId}/unfollow/`, {
                                        method: 'POST',
                                        headers: {
                                            'accept': '*/*',
                                            'accept-encoding': 'gzip, deflate, br',
                                        ...
                                        .then(text => {
                                            ...
                                            console.log(`UNFOLLOW dataUser ${currentUserId}, i = ${i}, time=${Date()}`);

                                            userId.shift();

                                            if (end_cursor && userId.length === 0) {
                                                new Promise((resolve) => {
                                                    ...
                                                    fetch(`${instUrl}graphql/query/?query_hash=c56ee0ae1f89cdbd1c89e2bc6b8f3d18&variables=${variables}`, {
                                                        ...
                                                        .then(text => {
                                                            ...
                                                            const parsedResponse = JSON.parse(text);
                                                            // const userId = parsedResponse.data.user.edge_follow.edges;
                                                            userId = parsedResponse.data.user.edge_follow.edges;
                                                            end_cursor = parsedResponse.data.user.edge_follow.page_info.end_cursor;
